...
2. **Reference** CSS custom property names from that file (e.g. `--color-primary`, `--spacing-md`). Do not invent parallel token values. In CSS and inline var(--…) usage, every custom property name must exist under design-system.json → tokens (any section).
